class UserNotifier < ActionMailer::Base
  ...
  def donation_thanks_notification(donor, pledge)
    setup_email(donor)
    @subject      += :notification_donation_thanks_subject.l
    @body[:url]    = "#{APP_URL}"
    @body[:amount] = pledge.total_amount
    @body[:date]   = pledge.created_at
  end

  def friends_invitation(invite, from)
    setup_email(from)
    @from           = "The #{AppConfig.community_name} Team on behalf of #{from.first_name} #{from.last_name} <#{AppConfig.support_email}>"
    @recipients     = nil
    @bcc            = invite.emails
    @subject        = "[#{AppConfig.community_name}] #{invite.title}"
    @body[:url]     = "#{APP_URL}"
  ...
  def reset_password(user)
    setup_email(user)
    @subject    += "#{AppConfig.community_name} Account information"
    @body[:url]  = "#{APP_URL}"
  end
