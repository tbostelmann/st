...
class User < ActiveRecord::Base
  ...
  # This is gross - at the very least table-drive this (and use this as a behavior spec)
  after_create {|user|
    if user.instance_of?(Donor) && SaveTogether::Notifications.donors ||
        user.instance_of?(Saver) && SaveTogether::Notifications.savers ||
        user.instance_of?(Organization) && SaveTogether::Notifications.organizations then
  ...
  # ST validation changes - these replace the validations commented out above:
  validates_format_of       :login, :with => /^([^@\s]+)@((?:[-a-z0-9A-Z]+\.)+[a-zA-Z]{2,})$/
